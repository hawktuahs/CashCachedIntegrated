...
import com.bt.product.util.CurrencyConverter;
...
public class ProductService {
    ...
    public ProductResponse getProductByCode(String productCode, String userCurrency) {
        Product product = productRepository.findByProductCode(productCode)
                .orElseThrow(() -> new ProductNotFoundException("Product not found with code: " + productCode));
        return mapToProductResponse(product, userCurrency);
    }
    ...
    public List<ProductResponse> getAllProducts(String userCurrency) {
        return productRepository.findAll().stream()
                .map(product -> mapToProductResponse(product, userCurrency))
                .collect(Collectors.toList());
    }

    public List<ProductResponse> searchProducts(ProductSearchRequest searchRequest) {
        return searchProducts(searchRequest, null);
    }

    public List<ProductResponse> searchProducts(ProductSearchRequest searchRequest, String userCurrency) {
    ...
    private ProductResponse mapToProductResponse(Product product, String userCurrency) {
        String baseCurrency = product.getCurrency() != null ? product.getCurrency().name() : currencyConverter.getBaseCurrency();
        String displayCurrency = userCurrency != null ? userCurrency : baseCurrency;

        BigDecimal minAmount = product.getMinAmount();
        BigDecimal maxAmount = product.getMaxAmount();

        if (minAmount != null) {
            minAmount = currencyConverter.convert(minAmount, baseCurrency, displayCurrency);
        }
        if (maxAmount != null) {
            maxAmount = currencyConverter.convert(maxAmount, baseCurrency, displayCurrency);
        }
